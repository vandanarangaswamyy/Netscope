...
def test_github_oidc_trust_is_restricted_to_repo_main_and_audience():
    ...
    assert 'default     = "181282565"' in variables_tf
    assert 'default     = "Netscope"' in variables_tf
    assert 'default     = "1308859104"' in variables_tf
    assert 'default     = "main"' in variables_tf
    assert 'default     = "repo:vandanarangaswamyy@181282565/Netscope@1308859104"' in variables_tf
    assert 'github_subject_claim = "${var.github_subject_claim_prefix}:ref:refs/heads/${var.github_branch}"' in main_tf
    assert 'variable = "${local.oidc_provider_host}:aud"' in main_tf
    assert 'variable = "${local.oidc_provider_host}:sub"' in main_tf
    assert "local.github_subject_claim" in main_tf


def test_github_subject_claim_rejects_wildcards():
    variables_tf = read_tf("variables.tf")

    assert "!strcontains(var.github_subject_claim_prefix, \"*\")" in variables_tf
    assert "!strcontains(var.github_subject_claim_prefix, \"?\")" in variables_tf
    assert "!strcontains(var.github_branch, \"*\")" in variables_tf
    assert "!strcontains(var.github_branch, \"?\")" in variables_tf
    assert 'can(regex("^[0-9]+$", var.github_owner_id))' in variables_tf
    assert 'can(regex("^[0-9]+$", var.github_repository_id))' in variables_tf
...
def test_github_secret_setup_is_documented():
    ...
    assert "/repos/vandanarangaswamyy/Netscope/actions/oidc/customization/sub" in readme
    assert "sub_claim_prefix" in readme
